...
**CLI abstraction layer:** The CLI uses user-facing slugs, not API codes. `--type pro` maps to `teacher_type: 1`. `--category conversation` maps to `course_category: ["CA005"]`. `--weekday mon,tue` maps to `weekday: [1,2]`. `--courses`/`--stats`/`--packages`/`--schedule` on `teacher` command are opt-in detail flags (default shows compact profile). `--sort`/`--limit`/`--all` on `search` command are client-side post-processing (API doesn't support sort). `--upcoming` on `lessons` includes all non-completed/non-canceled groups (action_required, waiting, unscheduled, upcoming) — not just the `upcoming` group. `--timezone` on `schedule` and `teacher --schedule` converts UTC slots to user's IANA timezone (priority: `--timezone` flag > `config.timezone_iana` from login > `DEFAULT_TIMEZONE`). `--page`/`--page-size` on `reviews` for pagination. `--duration` on `book` (with `--course-id` when the teacher has multiple courses in the language) resolves the matching `course_price_id` from the teacher's `price_list` via `resolveCoursePrice` in `transforms/teacher.ts` — without it the API books the first price entry (usually 30min). Explicit `--course-price-id` is validated against the price list (a course ID passed here errors with all valid IDs). `--duration` on `schedule` expands free blocks into bookable 30-min-grid starts where start + duration fits (via `expandStarts` in `transforms/schedule.ts`). Date/time display: shared `formatDateTime` in `lib/time-ago.ts` (24h, weekday, "at" separator, year for past events). Relative time via `timeAgo`/`timeUntil` in parentheses. Duration via `formatDuration` ("30min", "1h15min"). Lessons output shows group label (Completed, Upcoming, Action required, etc.) with distinct icons per group. Translation happens in `transforms/` via maps in `constants.ts` (cents→dollars, tag codes→names, 15-min units→minutes, level numbers→CEFR). `docs/api-reference.md` documents raw API codes — that's API-level truth for service code. The transforms layer is the domain truth. The CLI layer is the user-facing truth.
